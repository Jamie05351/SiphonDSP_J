...
class ParametricEqSurface(context: Context?, attrs: AttributeSet?) : View(context, attrs) {
    ...
    init {
        ...
        controlBarText.textSize = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 11f, getContext().resources.displayMetrics)
    ...
    override fun onSaveInstanceState() = bundleOf(
        "super" to super.onSaveInstanceState(), STATE_FREQ to curveFreqs,
        STATE_LEFT_GAIN to leftCurveGains, STATE_RIGHT_GAIN to rightCurveGains, STATE_PREAMP to preampDb,
    )
    ...
    override fun onAttachedToWindow() { super.onAttachedToWindow(); setLayerType(LAYER_TYPE_HARDWARE, null) }

    override fun onLayout(changed: Boolean, left: Int, top: Int, right: Int, bottom: Int) {
        super.onLayout(changed, left, top, right, bottom)
        viewWidth = (right - left).toFloat(); viewHeight = (bottom - top).toFloat()
        frequencyResponseBg.shader = LinearGradient(
            0f, 0f, 0f, viewHeight,
            intArrayOf(leftResponsePaint.color, getColor(android.R.color.transparent)),
            floatArrayOf(0.0f, 1f), Shader.TileMode.CLAMP,
        )
    }
    ...
    override fun onDraw(canvas: android.graphics.Canvas) {
        ...
        for (scale in FREQ_SCALE) canvas.drawText(scale.prettyNumberFormat(), projectX(scale) * viewWidth, viewHeight - 16, controlBarText)
        var db = minDb + 3
        while (db <= maxDb - 3) {
        ...
        if (!leftPath.isEmpty) {
            responseBgPath.addPath(leftPath); responseBgPath.offset(0f, -4f)
            ...
            } else { responseBgPath.lineTo(viewWidth, viewHeight); responseBgPath.lineTo(0f, viewHeight) }
            responseBgPath.close(); canvas.drawPath(responseBgPath, frequencyResponseBg)
        }
        canvas.drawPath(leftPath, leftResponsePaint); canvas.drawPath(rightPath, rightResponsePaint)
        canvas.drawText("L", 24f, 28f, controlBarText.apply { textAlign = Paint.Align.LEFT })
        canvas.drawText("R - -", 56f, 28f, controlBarText); controlBarText.textAlign = Paint.Align.CENTER
    }

    private fun buildPath(path: Path, gains: DoubleArray) {
        if (curveFreqs.isNotEmpty() && gains.size == curveFreqs.size) {
            path.moveTo(projectX(curveFreqs[0]) * viewWidth, projectY(gains[0].toFloat() + preampDb.toFloat()) * viewHeight)
            for (i in 1 until curveFreqs.size) path.lineTo(
                projectX(curveFreqs[i]) * viewWidth,
                projectY(gains[i].toFloat() + preampDb.toFloat()) * viewHeight,
            )
        } else {
            val y = projectY(preampDb.toFloat()) * viewHeight; path.moveTo(0f, y); path.lineTo(viewWidth, y)
        }
    }
    ...
    fun setPreampDb(preampDb: Double) { this.preampDb = preampDb; updateDbRange(); postInvalidate() }
    ...
    private fun updateDbRange() {
        ...
        minDb = floor(minOf(minGain, -15.0)).toInt(); maxDb = ceil(maxOf(maxGain, 15.0)).toInt()
    }

    private fun projectX(frequency: Double): Float = ((ln(frequency) - ln(MIN_FREQ)) / (ln(MAX_FREQ) - ln(MIN_FREQ))).toFloat()
    private fun projectY(db: Float): Float = 1.0f - (db - minDb) / (maxDb - minDb)
