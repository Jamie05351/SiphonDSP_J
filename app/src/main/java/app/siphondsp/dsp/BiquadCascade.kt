...
internal class BiquadCascade(maxSections: Int) {
    ...
    /**
     * NativeBmwRouting::AllPassSection::rebuild. Native validates all-pass frequency against
     * the true Nyquist boundary (< sampleRate * 0.5), unlike the 0.49 safety clamp used by
     * the crossover coefficient builders.
     */
    fun addAllPass(enabled: Boolean, secondOrder: Boolean, frequencyHz: Double, q: Double, sampleRate: Double) {
        if (!enabled || !frequencyHz.isFinite() || frequencyHz < FREQ_MIN || frequencyHz >= sampleRate * ALL_PASS_NYQUIST_FRACTION ||
    ...
    companion object {
        ...
        private const val FILTER_NYQUIST_FRACTION = .49
        private const val ALL_PASS_NYQUIST_FRACTION = .5
