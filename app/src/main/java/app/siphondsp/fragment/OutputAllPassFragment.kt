...
import app.siphondsp.compose.theme.ThemeSwatchCheck
...
class OutputAllPassFragment : Fragment() {
    ...
    private fun rebuild() {
        ...
        // TEMPORARY (Phase 2 theme verification): stacks the theme swatch check above the
        // real pager, same pattern as Phase 1's ComposeSmokeTest. Remove this block, both
        // imports above, and ThemeSwatchCheck.kt once every swatch color is confirmed correct
        // on-device.
        val themeCheckView = ComposeView(requireContext()).apply {
            setContent { ThemeSwatchCheck() }
        }

        val stack = LinearLayout(requireContext()).apply {
            orientation = LinearLayout.VERTICAL
        }
        stack.addView(themeCheckView, LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT))
